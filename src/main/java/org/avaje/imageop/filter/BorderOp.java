...
public class BorderOp extends ImageOp {
   ...
   */
  public BorderOp(int leftBorder, int topBorder, int rightBorder, int bottomBorder, Paint borderPaint, int originImageType) {
    this.leftBorder = leftBorder;
    this.topBorder = topBorder;
    this.rightBorder = rightBorder;
    this.bottomBorder = bottomBorder;
    this.borderPaint = borderPaint;
    this.originImageType = originImageType;
  }
  ...
  public BufferedImage filter(BufferedImage src, BufferedImage dst) {
    ...
    if (dst == null) {
      int type = src.getType() != 0 ? src.getType() : originImageType;
    ...
    if (borderPaint != null) {
      ...
      if (rightBorder > 0)
        g.fillRect(width + leftBorder, 0, rightBorder, height);
      if (topBorder > 0)
        g.fillRect(leftBorder, 0, width - leftBorder - rightBorder, topBorder);
      if (bottomBorder > 0)
        g.fillRect(leftBorder, height + topBorder, width - leftBorder - rightBorder, bottomBorder);
    }
